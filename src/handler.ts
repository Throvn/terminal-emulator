import { Helper, Command } from './core'
...
function checkCommands(rawCmd: string): void {
  ...
  const command: Command = Commands.find(cmd[0]);
...
const checkInput = function (evt: KeyboardEvent): void {
  ...
  if (evt.key === "Enter") {
    ...
    $cmd.parentElement.parentElement.insertAdjacentHTML(
      ...
      `
              <div class="line">
                  ...
                  <div class="window-input" contenteditable="true"></div>
              </div>
          `
    );
    ...
    if ($cmd.innerText.charAt(0) === $cmd.innerText.charAt(0).toUpperCase()) {
      $cmd.innerText =
        $cmd.innerText.charAt(0).toLowerCase() + $cmd.innerText.substring(1);
      Helper.placeCaretAtEnd($cmd);
      $cmd.focus();
    }
  }
